...
public class MyStorage {
    ...
    public static int isValidIndex(String[] strArr){
        int currentIdx = -1;
    ...
    public static int selectMenu(Scanner s){
        ...
        // 사용자로부터 정수 입력을 받아 변수 select에 저장합니다.
        if(isValidNumber(selectStr)){
            select = Integer.parseInt(selectStr);
        }

        return select;  // 입력 받은 정수를 반환합니다.
    }
    // string 배열에서 Empty를 만날 때 인덱스 값을 가져오는 메소드
    ...
    static void prod_input(Scanner s){
        ...
        int currentIndex = isValidIndex(products);

        if (currentIndex != -1) {
            for(int i =0 ; i <currentIndex; i++){
                if(productName.equals(products[i])){
                    System.out.println("[Error] 이름이 같은 제품은 등록할 수 없습니다");
                    return;
                }
            }
            ...
            System.out.println("[Error] 더 이상 제품을 등록할 수 없습니다.");
            return;
        }
     ...
     * 반환 타입은 void로, 별도의 반환 값이 없습니다.
     */
    public static int getProductIndex(String[] product, int maxLength, String input ){
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        int cnt = 0;
        // 수량 입력 받기
        String cntStr = s.nextLine();
        if(isValidNumber(cntStr)){
...
//        int foundIdx = -1;  // 찾은 인덱스 초기화
//        for (int i = 0; i < products.length; ++i){
//            if (productName.equals(products[i])){  // 입력한 제품명과 일치하는 제품을 찾으면
//                foundIdx = i;  // 인덱스 저장
//                break;  // 반복 중단
//            }
//        }
...
//            return;
//        }
        int productIndex = getProductIndex(products, isValidIndex(products), productName);
...
//            return;
        ...
        if(cnt > counts[productIndex]){
            System.out.println("[Error] 남은 수량보다 더 많이 출고할 수 없습니다.");
            return;
        }
        counts[productIndex] -= cnt;  // 제품의 수량을 증가
